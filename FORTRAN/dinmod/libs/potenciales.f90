...
contains
  ...
  ! 
  function poten_lj_vec (this) result(Pot)
    class (Lenard_Jones) :: this
  ...
  !
  function delta_poten_lj_vec  (this, i, ri_vec) result(deltaPot)
    class (Lenard_Jones) :: this
    ...
    vn = 0.0_dp
    do j = 1, i - 1
      vo     = vo + this % kernel(j, ri_vec)   ! Energía potencial
      vn     = vn + this % kernel(j, gR(:,i))   ! Energía potencial
    enddo
  ...
  !
  function kernel_pot (this, j, ri_vec) result (Pot)
    class (Lenard_Jones) :: this
    real(dp), intent(in), dimension(3)  :: ri_vec
    integer, intent(in) :: j
    real(dp)                :: r2ij      ! Módulo cuadrado de la distancia rij
