/* 
   ...
   compile using "gcc -o mdlj_berp mdlj_berp.c -lm -lgsl"
...
void usage ( void ) {
  fprintf(stdout,"mdlj_berp usage:\n");
  fprintf(stdout,"mdlj_berp [options]\n\n");
  ...
  fprintf(stdout,"\t -P [real]\t\tSetpoint Pressure\n");
  fprintf(stdout,"\t -tau [real]\t\tRise time of barostat\n");
...
int main ( int argc, char * argv[] ) {
  ...
  int N=216,c,a;
  double L=0.0;
  double rho=0.5, P0 = 1.0, P, tau=1.0, rc2 = 1.e20;
  double vir, vir_sum, pcor, V, pj_P, pj_tau;
  double PE, KE, TE, ecor, ecut, T0=0.0, TE0;
  double rr3,dt=0.001, dt2, mu=0.1;
  int i,j,s;
  ...
  fprintf(stdout,"# Berendsen-Barostat MD Simulation"
	  " of a Lennard-Jones fluid\n");
  fprintf(stdout,"# L = %.5lf; rho = %.5lf; N = %i; rc = %.5lf\n",
	  L,rho,N,sqrt(rc2));
  fprintf(stdout,"# nSteps %i, seed %d, dt %.5lf, P %.5lf, tau %.5lf\n",
	  nSteps,Seed,dt,P,tau);
  ...
  for (s=0;s<nSteps;s++) {
    ...
    mu = pow((1-dt/tau*(P0-P)),0.33333333333);
    for (i=0;i<N;i++) {
      rx[i]*=mu;
      ry[i]*=mu;
      rz[i]*=mu;
    ...
    pcor = use_e_corr?16.0/3.0*M_PI*rho*rho*(2./3.*rr3*rr3*rr3-rr3):0.0;
    TE=PE+KE;
    fprintf(stdout,"%i %.5lf %.5lf %.5lf %.5lf %.5le %.5lf %.5lf %.5lf\n",
	    s,s*dt,PE,KE,TE,(TE-TE0)/TE0,KE*2/3./N,rho*KE*2./3./N+vir/3.0/V,mu);
    if (!(s%fSamp)) {
